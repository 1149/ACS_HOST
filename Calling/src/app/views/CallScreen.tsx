...
export const CallScreen = (props: CallScreenProps): JSX.Element => {
  ...
  const [remainingTime, setRemainingTime] = useState(MAX_CALL_DURATION_MS);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      const newRemainingTime = MAX_CALL_DURATION_MS - elapsedTime;
  
      if (newRemainingTime <= 0) {
        // clearInterval(interval); 
        console.log("Call ending due to time limit");
        alert("Call ending due to 30 minute time limit");
        clearInterval(interval); // Clear interval to prevent further execution
        window.location.reload(); // End call by refreshing or handling via API
