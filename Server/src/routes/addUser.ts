...
router.post('/:groupId', async function (req, res, next) {
  ...
  const threadId = req.params['groupId'];
  ...
  try {
    ...
    // List the participants in the thread
    const participants = [];
    ...
    debugger;
    // If there are already 2 participants, reject the request
    if (currentParticipantsCount >= 2) {
      res.status(400).send({ message: "Call is full. Only 2 participants are allowed." });
      return;
    }
